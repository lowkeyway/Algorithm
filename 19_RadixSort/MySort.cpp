...
bool MySort::radixSort(UP_DOWN Up_Down)
{
	int i = 0, j = 0;
	int max, maxRadix, radix, radixNum;
	...
	int* pRadix = new int[m_iCap];
	int* pTemp = new int[m_iCap];
	if (nullptr == pRadix || nullptr == pTemp)
	{
		cout << "New pRadix or pTemp Fail!" << endl;
		return false;
	}

	m_ulCount = 0;
	memset(pRadix, 0, sizeof(pRadix[0]) * m_iCap);
	memset(pTemp, 0, sizeof(pTemp[0]) * m_iCap);
	memcpy(pRadix, m_ipArray, sizeof(pRadix[0]) * m_iCap);

	// 1. Get max and its radix
	max = pRadix[0];
	for (i = 0; i < m_iCap; i++)
	{
		m_ulCount++;
	...
	for (i = 0; i < maxRadix; i++)
	{
		...
		// count sort
		for (int index = 0; index < m_iCap; index++)
		{
			m_ulCount++;
			radixNum = (pRadix[index] / radix) % 10;
			count[radixNum]++;
		}
		...
		// map to pTemp
		for (int k = m_iCap - 1; k >= 0; k--)
		{
			m_ulCount++;
			radixNum = (pRadix[k] / radix) % 10;
			pTemp[count[radixNum] - 1] = pRadix[k];
			count[radixNum]--;
		}

		// Copy to pRadix
		for (int k = 0; k < m_iCap; k++)
		{
			m_ulCount++;
			pRadix[k] = pTemp[k];
		}
	...
	if (DOWN == Up_Down)
	{
		...
		{
			m_ulCount++;
			pRadix[m_iCap - k - 1] = pTemp[k];
		}
	}

	showArray(pRadix, m_iCap);
	printf("Radix sort takes [%lu] Times when n = [%d] \n", m_ulCount, m_iCap);
